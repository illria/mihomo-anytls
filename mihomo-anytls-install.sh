...
rand_secret(){
  if has openssl; then openssl rand -hex 16; else date +%s%N | sha256sum | cut -c1-32; fi
}

make_uuid(){
  if [ -r /proc/sys/kernel/random/uuid ]; then cat /proc/sys/kernel/random/uuid; elif has uuidgen; then uuidgen | tr 'A-Z' 'a-z'; else printf '%s-%s-%s-%s-%s\n' "$(rand_secret|cut -c1-8)" "$(rand_secret|cut -c1-4)" "$(rand_secret|cut -c1-4)" "$(rand_secret|cut -c1-4)" "$(rand_secret|cut -c1-12)"; fi
}
...
detect_pkg(){
  if has apt-get; then PKG_MANAGER=apt; elif has dnf; then PKG_MANAGER=dnf; elif has yum; then PKG_MANAGER=yum; elif has apk; then PKG_MANAGER=apk; elif has pacman; then PKG_MANAGER=pacman; elif has zypper; then PKG_MANAGER=zypper; elif has opkg; then PKG_MANAGER=opkg; else PKG_MANAGER=unknown; fi
}

detect_init(){
  if has systemctl && [ -d /run/systemd/system ]; then INIT_SYSTEM=systemd; elif has rc-service; then INIT_SYSTEM=openrc; elif [ -x /etc/init.d/docker ]; then INIT_SYSTEM=initd; else INIT_SYSTEM=unknown; fi
}
...
start_docker(){
  ...
  case "$INIT_SYSTEM" in
    ...
    openrc) rc-update add docker default >/dev/null 2>&1 || true; rc-service docker start >/dev/null 2>&1 || true ;;
    initd) /etc/init.d/docker start >/dev/null 2>&1 || true ;;
  esac
}
...
install_docker(){
  if ! has docker; then
    info "安装 Docker"
    case "$PKG_MANAGER" in
      apt) DEBIAN_FRONTEND=noninteractive apt-get install -y docker.io docker-compose-plugin || true ;;
      dnf) dnf install -y docker docker-compose-plugin || true ;;
      yum) yum install -y docker docker-compose-plugin || true ;;
      apk) apk add --no-cache docker docker-cli-compose || true ;;
      pacman) pacman -Sy --noconfirm --needed docker docker-compose || true ;;
      zypper) zypper --non-interactive install docker docker-compose || true ;;
      opkg) opkg install dockerd docker docker-compose || true ;;
    esac
  fi
  has docker || die "Docker 不存在，请先手动安装 Docker。"
  start_docker
...
install_acme(){
  [ -n "$ACME_EMAIL" ] || ask ACME_EMAIL "请输入 acme.sh 注册邮箱" "admin@$DOMAIN"
  if [ ! -x "$HOME/.acme.sh/acme.sh" ]; then curl https://get.acme.sh | sh -s email="$ACME_EMAIL"; fi
...
install_acme_cert(){
  mkdir -p "$CERT_DIR"
  "$HOME/.acme.sh/acme.sh" --install-cert -d "$DOMAIN" --ecc --key-file "$KEY_FILE" --fullchain-file "$CERT_FILE" --reloadcmd "$(reload_cmd)" || \
  "$HOME/.acme.sh/acme.sh" --install-cert -d "$DOMAIN" --key-file "$KEY_FILE" --fullchain-file "$CERT_FILE" --reloadcmd "$(reload_cmd)"
  chmod 600 "$KEY_FILE"; chmod 644 "$CERT_FILE"
}

self_cert(){
  mkdir -p "$CERT_DIR"; info "生成自签证书：$DOMAIN"
  openssl req -x509 -nodes -newkey rsa:2048 -days 3650 -keyout "$KEY_FILE" -out "$CERT_FILE" -subj "/CN=$DOMAIN" -addext "subjectAltName=DNS:$DOMAIN" >/dev/null 2>&1 || \
  openssl req -x509 -nodes -newkey rsa:2048 -days 3650 -keyout "$KEY_FILE" -out "$CERT_FILE" -subj "/CN=$DOMAIN" >/dev/null 2>&1
  chmod 600 "$KEY_FILE"; chmod 644 "$CERT_FILE"; SKIP_CERT_VERIFY=true
}

custom_cert(){
  local c k; ask c "请输入证书 fullchain/cert 路径"; ask k "请输入私钥 key 路径"; [ -f "$c" ] || die "证书不存在：$c"; [ -f "$k" ] || die "私钥不存在：$k"
  mkdir -p "$CERT_DIR"; cp -f "$c" "$CERT_FILE"; cp -f "$k" "$KEY_FILE"; chmod 600 "$KEY_FILE"; chmod 644 "$CERT_FILE"; SKIP_CERT_VERIFY=false
}

issue_80(){
  install_acme
  if has ss && ss -ltn | awk '{print $4}' | grep -Eq '(^|:)80$'; then warn "80/tcp 已被占用"; confirm "仍继续尝试吗" n || die "已取消"; fi
  "$HOME/.acme.sh/acme.sh" --issue --standalone -d "$DOMAIN" --keylength ec-256 --server letsencrypt
  install_acme_cert; SKIP_CERT_VERIFY=false
}

issue_cf(){
  local token account zone; install_acme; ask token "请输入 Cloudflare API Token"; ask account "请输入 Cloudflare Account ID"; read -r -p "请输入 Cloudflare Zone ID [可留空]: " zone
  export CF_Token="$token" CF_Account_ID="$account"; [ -n "$zone" ] && export CF_Zone_ID="$zone"
  "$HOME/.acme.sh/acme.sh" --issue --dns dns_cf -d "$DOMAIN" --keylength ec-256 --server letsencrypt
  install_acme_cert; SKIP_CERT_VERIFY=false
}

choose_cert(){
  echo; echo "请选择证书方式："; echo "  1) 自签证书"; echo "  2) 自定义证书路径"; echo "  3) 80 端口申请 Let's Encrypt"; echo "  4) Cloudflare DNS 验证证书"
  read -r -p "输入序号 [4]: " CERT_MODE; CERT_MODE=${CERT_MODE:-4}
  case "$CERT_MODE" in 1) self_cert;; 2) custom_cert;; 3) issue_80;; 4) issue_cf;; *) die "无效证书方式";; esac
}

choose_core(){
  echo; echo "请选择内核："; echo "  1) mihomo"; echo "  2) sing-box"; read -r -p "输入序号或名称 [1]: " x; x=${x:-1}
  case "$x" in 1|mihomo|mihome) CORE=mihomo;; 2|sing-box|singbox) CORE=sing-box;; *) die "无效内核：$x";; esac
}

choose_install(){
  echo; echo "请选择安装方式："; echo "  1) Docker 模式（推荐）"; echo "  2) 裸机 systemd 模式"; read -r -p "输入序号 [1]: " x; x=${x:-1}
  case "$x" in 1|docker) INSTALL_MODE=docker;; 2|systemd) INSTALL_MODE=systemd;; *) die "无效安装方式";; esac
  detect_init
  if [ "$INSTALL_MODE" = systemd ] && [ "$INIT_SYSTEM" != systemd ]; then warn "当前不是 systemd：$INIT_SYSTEM"; confirm "改用 Docker 模式吗" y && INSTALL_MODE=docker; fi
}

choose_singbox_version(){
  [ "$CORE" = sing-box ] || return 0
  local old; old="$(existing_singbox_ver)"
  echo; echo "请选择 sing-box 版本："; echo "  1) latest（支持 AnyTLS）"; echo "  2) 1.11.x 兼容版，默认 v$DEFAULT_SINGBOX_111（隐藏 AnyTLS）"; echo "  3) 自定义版本"; echo "  4) 使用系统现有版本 ${old:+($old)}"
  read -r -p "输入序号 [1]: " x; x=${x:-1}
  case "$x" in
    1) SINGBOX_MODE=latest; SINGBOX_VERSION=latest;;
    2) SINGBOX_MODE=1.11.x; SINGBOX_VERSION=$DEFAULT_SINGBOX_111;;
    3) SINGBOX_MODE=custom; ask SINGBOX_VERSION "请输入版本，例如 1.12.0 或 1.11.15"; SINGBOX_VERSION=${SINGBOX_VERSION#v};;
    4) [ -n "$old" ] || die "系统未检测到 sing-box"; SINGBOX_MODE=existing; SINGBOX_VERSION=$old;;
    *) die "无效版本选择";;
  esac
}

choose_protocol(){
  if [ "$CORE" = mihomo ]; then PROTOCOL=anytls; NEED_PROTO=tcp; return; fi
  local ok=false; supports_anytls "$SINGBOX_VERSION" && ok=true
  echo; echo "请选择协议："
  if [ "$ok" = true ]; then
    echo "  1) AnyTLS"; echo "  2) Hysteria2"; echo "  3) TUIC v5"; echo "  4) Trojan TLS"; read -r -p "输入序号 [1]: " x; x=${x:-1}
    case "$x" in 1|anytls) PROTOCOL=anytls; NEED_PROTO=tcp;; 2|hysteria2|hy2) PROTOCOL=hysteria2; NEED_PROTO=udp;; 3|tuic) PROTOCOL=tuic; NEED_PROTO=udp;; 4|trojan) PROTOCOL=trojan; NEED_PROTO=tcp;; *) die "无效协议";; esac
  else
    warn "sing-box $SINGBOX_VERSION 低于 $ANYTLS_MIN_SINGBOX，隐藏 AnyTLS。"
    echo "  1) Hysteria2"; echo "  2) TUIC v5"; echo "  3) Trojan TLS"; read -r -p "输入序号 [1]: " x; x=${x:-1}
    case "$x" in 1|hysteria2|hy2) PROTOCOL=hysteria2; NEED_PROTO=udp;; 2|tuic) PROTOCOL=tuic; NEED_PROTO=udp;; 3|trojan) PROTOCOL=trojan; NEED_PROTO=tcp;; anytls) die "当前 sing-box 版本不支持 AnyTLS";; *) die "无效协议";; esac
  fi
}
...
write_singbox_config(){
  ...
  case "$PROTOCOL" in
    anytls)
      inbound='{"type":"anytls","tag":"anytls-in","listen":"::","listen_port":'"$PORT"',"users":[{"password":"'"$PASSWORD"'"}],"padding_scheme":"","tls":{"enabled":true,"server_name":"'"$DOMAIN"'","certificate_path":"'"$CERT_FILE"'","key_path":"'"$KEY_FILE"'"}}'
      mihomo_type=anytls; sing_type=anytls;;
    hysteria2)
      inbound='{"type":"hysteria2","tag":"hy2-in","listen":"::","listen_port":'"$PORT"',"users":[{"name":"'"$USER_NAME"'","password":"'"$PASSWORD"'"}],"tls":{"enabled":true,"server_name":"'"$DOMAIN"'","certificate_path":"'"$CERT_FILE"'","key_path":"'"$KEY_FILE"'"}}'
      mihomo_type=hysteria2; sing_type=hysteria2;;
    tuic)
      UUID_VALUE=${UUID_VALUE:-$(make_uuid)}
      inbound='{"type":"tuic","tag":"tuic-in","listen":"::","listen_port":'"$PORT"',"users":[{"uuid":"'"$UUID_VALUE"'","password":"'"$PASSWORD"'"}],"congestion_control":"bbr","tls":{"enabled":true,"server_name":"'"$DOMAIN"'","certificate_path":"'"$CERT_FILE"'","key_path":"'"$KEY_FILE"'"}}'
      mihomo_type=tuic; sing_type=tuic;;
    trojan)
      inbound='{"type":"trojan","tag":"trojan-in","listen":"::","listen_port":'"$PORT"',"users":[{"name":"'"$USER_NAME"'","password":"'"$PASSWORD"'"}],"tls":{"enabled":true,"server_name":"'"$DOMAIN"'","certificate_path":"'"$CERT_FILE"'","key_path":"'"$KEY_FILE"'"}}'
      mihomo_type=trojan; sing_type=trojan;;
  esac
  cat > "$CONFIG_FILE" <<EOF
...
install_systemd(){
  [ "$INIT_SYSTEM" = systemd ] || die "裸机模式需要 systemd"
  if [ "$CORE" = mihomo ]; then
...
EOF
    systemctl daemon-reload; systemctl enable --now mihomo; systemctl restart mihomo
  else
    [ "$SINGBOX_MODE" = existing ] && BIN_PATH="$(command -v sing-box)" || download_singbox "$BIN_PATH" "$SINGBOX_VERSION"
    cat > "$SERVICE_FILE" <<EOF
[Unit]
...
compose_up(){ if docker compose version >/dev/null 2>&1; then (cd "$WORK_DIR" && docker compose up -d --build); elif has docker-compose; then (cd "$WORK_DIR" && docker-compose up -d --build); else return 1; fi; }

install_docker_mode(){
  install_docker; mkdir -p "$WORK_DIR"
  if [ "$CORE" = mihomo ]; then
...
EOF
  if ! compose_up; then
    ...
    if [ "$CORE" = mihomo ]; then docker run -d --name "$CONTAINER_NAME" --restart unless-stopped --network host -v "$BASE_DIR:$BASE_DIR:ro" "$IMAGE_NAME" -d "$BASE_DIR"; else docker run -d --name "$CONTAINER_NAME" --restart unless-stopped --network host -v "$BASE_DIR:$BASE_DIR:ro" "$IMAGE_NAME" run -c "$CONFIG_FILE"; fi
  fi
}

firewall_hint(){
  if has ufw && ufw status 2>/dev/null | grep -q 'Status: active'; then confirm "ufw 放行 $PORT/$NEED_PROTO 吗" y && ufw allow "$PORT/$NEED_PROTO"; elif has firewall-cmd && firewall-cmd --state >/dev/null 2>&1; then confirm "firewalld 放行 $PORT/$NEED_PROTO 吗" y && firewall-cmd --permanent --add-port="$PORT/$NEED_PROTO" && firewall-cmd --reload; else warn "请确认云安全组/软路由防火墙已放行 $PORT/$NEED_PROTO"; fi
}
...
main(){
  ...
  choose_core; choose_install; choose_singbox_version; choose_protocol; prepare_paths; collect_inputs; choose_cert; write_config
  [ "$INSTALL_MODE" = docker ] && install_docker_mode || install_systemd
  firewall_hint; summary
}
