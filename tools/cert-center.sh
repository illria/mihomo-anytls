...
is_readable_cert(){ [ -s "$1" ] && openssl x509 -in "$1" -noout >/dev/null 2>&1; }
...
days_left(){
  ...
  is_readable_cert "$1" || { echo "invalid"; return; }
  end="$(cert_end "$1")"
...
domains_of(){
  ...
  is_readable_cert "$cert" || return 0
  text="$(cert_text "$cert")"
...
infer_domain_from_path(){
  ...
  d="${d%_ecc}"
  case "$d" in cert|certs|live|mihomo|sing-box|.) b="$(basename "$p")"; b="${b%.pem}"; b="${b%.cer}"; echo "$b" ;; *) echo "$d" ;; esac
}
...
emit_cert(){
  local kind="$1" source="$2" cert="$3" key="$4" dom days status dns domains
  [ -f "$cert" ] || return 1
  [ -f "$key" ] || return 1
  days="$(days_left "$cert")"
  domains="$(domains_of "$cert")"
  dom="$(printf '%s' "$domains" | awk '{print $1}')"
  [ -n "$dom" ] || dom="$(infer_domain_from_path "$cert")"
...
inventory(){
  ...
  for dir in /root/.acme.sh/*_ecc; do
    ...
    if [ -n "$cert" ]; then emit_cert "源证书" "acme.sh-ecc" "$cert" "$key" || true; else emit_record "acme-record-ecc" "$dir" "$d" "$key" || true; fi
  done
  for dir in /root/.acme.sh/*; do
    ...
    key="$dir/${d}.key"
    cert="$(acme_cert_in_dir "$dir" "$d" || true)"
    if [ -n "$cert" ]; then emit_cert "源证书" "acme.sh" "$cert" "$key" || true; else emit_record "acme-record" "$dir" "$d" "$key" || true; fi
  done
  ...
  for dir in /root/cert/* /root/certs/* /etc/ssl/* /etc/ssl/private/*; do
    [ -d "$dir" ] || continue
    d="$(basename "$dir")"
    cert=""
    key=""
    for c in "$dir/fullchain.pem" "$dir/fullchain.cer" "$dir/cert.pem" "$dir/${d}.pem" "$dir/${d}.cer"; do [ -f "$c" ] && { cert="$c"; break; }; done
    for k in "$dir/privkey.pem" "$dir/key.pem" "$dir/${d}.key"; do [ -f "$k" ] && { key="$k"; break; }; done
    [ -n "$cert" ] && [ -n "$key" ] && emit_cert "源证书" "custom-dir" "$cert" "$key" || true
  done
...
show_inventory(){
  ...
  echo "============================================================"
  if [ -z "$data" ]; then warn "没有发现本地证书或证书记录。"; return 1; fi
  printf '%-4s %-8s %-16s %-28s %-8s %-10s %-8s\n' "序号" "类型" "来源" "域名" "剩余" "状态" "DNS"
  echo "------------------------------------------------------------------------------------------------"
  i=1
  printf '%s\n' "$data" | while IFS='|' read -r kind source dom days status dns cert key; do
...
force_renew_selected(){
  local line kind source dom days status dns cert key target_cert target_key
  line="$(pick_row)" || return 1
  IFS='|' read -r kind source dom days status dns cert key <<EOF
$line
EOF
  case "$source" in
    acme.sh-ecc|acme-record-ecc|acme.sh|acme-record|certbot)
      ...
      curl -fsSL "$CERT_AUTO_URL?t=$(date +%s)" | bash -s -- "$dom" "$target_cert" "$target_key" 99999
      ;;
    custom-dir)
      err "custom-dir 是手动证书目录，不能自动续期，只能同步。"
      return 1
      ;;
