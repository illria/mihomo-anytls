...
install_command() (
  ...
  if ! install -m 755 "$tmp" "$BIN_MAIN"; then
    err "安装主命令失败。"
    return 1
  fi
  if ! install -m 755 "$tmp" "$BIN_SHORT"; then
    err "安装快捷命令失败。"
    return 1
  fi
...
17 4 * * * root bash -c 'tmp=\$(mktemp /tmp/mihomo-anytls-update.XXXXXX) || exit 1; if command -v curl >/dev/null 2>&1; then curl -fsSL -H "Cache-Control: no-cache" -H "Pragma: no-cache" "${INSTALL_URL}?t=\$(date +\%s)" -o "\$tmp"; else wget --no-cache -qO "\$tmp" "${INSTALL_URL}?t=\$(date +\%s)"; fi; rc=\$?; if [ \$rc -ne 0 ] || [ ! -s "\$tmp" ] || ! bash -n "\$tmp"; then rm -f -- "\$tmp"; exit 1; fi; install -m 755 "\$tmp" "$BIN_MAIN" && install -m 755 "\$tmp" "$BIN_SHORT"; rc=\$?; rm -f -- "\$tmp"; exit \$rc' >> "$LOG_FILE" 2>&1
EOF
  chmod 644 "$CRON_FILE"
}
...
cron_file_valid(){
  [ -f "$CRON_FILE" ] && [ "$(cron_file_mode "$CRON_FILE")" = "644" ]
}
...
install_cron(){
  ...
  write_cron || { err "无法写入计划任务，未启用每日自动更新。"; return 1; }
  cron_file_valid || { err "计划任务文件不存在或权限不是 0644，未启用每日自动更新。"; return 1; }
...
status(){
  local main_ok=false short_ok=false cron_ok=false daemon_ok=false
  echo "主命令: $BIN_MAIN"
  ...
  if [ -f "$CRON_FILE" ]; then cron_ok=true; echo "  存在"; echo "下一次计划更新时间: $(next_schedule)"; else echo "  不存在"; echo "下一次计划更新时间: 未设置"; fi
  if cron_daemon_running; then daemon_ok=true; echo "cron daemon: 运行中"; else echo "cron daemon: 未运行"; fi
  if [ "$main_ok" = false ] && [ "$short_ok" = false ]; then
    echo "状态: 未安装"
  elif [ "$cron_ok" = false ]; then
